...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	if ($sql3['vorjahr_liga'] == $liga3['ids']) {
        ...
        if ($sql3['pokalrunde'] > 0) { $tmp_liga_cache .= ' [PT]'; }
        // Vorsaison
        $vorsaison=intval(GameTime::getSeason()-1);
        // Pokalsieger
        $ps1="SELECT sieger FROM ".$prefix."pokalsieger WHERE saison = '".$vorsaison."'";
        $ps2 = mysql_query($ps1);
        $ps3 = mysql_result($ps2, 0);
        if (strcmp($ps3, $sql3['name']) == 0) { $tmp_liga_cache .= ' [P]'; }
        // Cupsieger
        $cs1="SELECT sieger FROM ".$prefix."cupsieger WHERE saison = '".$vorsaison."' AND land = 'Deutschland'";
        $cs2 = mysql_query($cs1);
        $cs3 = mysql_result($cs2, 0);
        if (strcmp($cs3, $sql3['name']) == 0) { $tmp_liga_cache .= ' [C]'; }
	}
	else {
...
if (substr($liga3['name'], -1) == 1) {
	...
	$tmp_liga_cache .= '</p><p><strong>'._('In eckigen Klammern:').'</strong> '._('M=Meister, AU=Aufsteiger, C=Cupsieger, P=Pokalsieger, PT=Pokalteilnehmer').'</p>';
}
