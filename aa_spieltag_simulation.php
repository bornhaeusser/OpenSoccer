...
function kommentar($ersetzung, $typ) {
	...
	$formulierungen['start_game'] = array(
		...
		_('Heute, aus der Kabine, ihr Komentator Bella Rethie. Los geht\'s mit der ersten Halbzeit!'),
		_('Die Spieler stehe sich gegenüber. Der Schiri startet das Spiel. Anstoß!'),
		_('Heute für Sie am Rednerpult ihr Kommentator Horst Schrammel. Anstoß!')
    );
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
    ...
    if ($sql3['typ'] == 'Test') {
        $watcher_einkommen_h = round($watcher4*$watcher3['preis']/4);
        $watcher_einkommen_a = round($watcher4*$watcher3['preis']/4);
    }
	...
	elseif (preg_match('/^[a-mQ-R]*\s[b-sH-I]*$/', $sql3['team1'])) {
		$watcher_einkommen_h = round($watcher4*$watcher3['preis']/0.1);
    	$watcher_einkommen_a = 0;
	}
	elseif (preg_match('/^[a-mQ-R]*\s[b-sH-I]*$/', $sql3['team2'])) {
		$watcher_einkommen_h = round($watcher4*$watcher3['preis']);
    	$watcher_einkommen_a = round($watcher4*$watcher3['preis']/0.1);
	}
	else {
        $watcher_einkommen_h = round($watcher4*$watcher3['preis']);
        $watcher_einkommen_a = 0;
	...
	// Stadion ausverkauft Kommentar hinzufügen
	if ($watcher4 == $watcher3['plaetze'] && $watcher4 >= 50000) {
        $spielbericht .= '<p>Die Hütte brennt vor ausverkauftem Haus hier '.$stadiumPreposition.' '.$stadiumName.'!</p>';
	}
    elseif ($watcher4 == $watcher3['plaetze'] && $watcher4 >= 30000) {
    	$spielbericht .= '<p>Tolle Kulisse vor ausverkauftem Haus hier '.$stadiumPreposition.' '.$stadiumName.'!</p>';
	}
	elseif ($watcher4 == $watcher3['plaetze'] && $watcher4 < 30000) {
    	$spielbericht .= '<p>Ausverkauftes Haus hier '.$stadiumPreposition.' '.$stadiumName.'!</p>';
	}
	elseif ($watcher4 >= 90000) {
        $spielbericht .= '<p>Tolle Kulisse, knapp 100.000 Zuschauer hier '.$stadiumPreposition.' '.$stadiumName.'!</p>';
	}
	...
	$pflichtspielPlus = ", spiele = spiele+1, moral = moral+1.7";
	if ($to_simulate == 'Cup') {
		$pflichtspielPlus = ", moral = moral+1.8";
	}
	if ($to_simulate == 'Pokal') {
		$pflichtspielPlus = ", moral = moral+1.9";
	}
	if ($to_simulate == 'Test') {
