...
if ($loggedin == 1) {
    if (isset($_POST['typ']) && $cookie_id != CONFIG_DEMO_USER) { // && isset($_POST['laenge']) && isset($_POST['startgebot']) && isset($_POST['autorestart'])
        ...
        if ($sql2a > 0) {
            ...
            if ($sperre3 > 13) {
                if ($_POST['typ'] == 'Kauf') { // Verkaufen ins Ausland
                ...
                elseif ($_POST['typ'] = 999998) { // Verkaufen an Verein
                    $sql4 = "UPDATE ".$prefix."spieler SET transfermarkt = ".bigintval($_POST['typ']).", moral = 100-10 WHERE ids = '".$spieler_id."'";
                    $sql5 = mysql_query($sql4);
                }
            }
        }
