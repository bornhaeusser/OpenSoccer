...
if ($loggedin == 1 && $sql3['team'] == $cookie_team && $sql3['leiher'] == 'keiner' && $sql3['marktwert'] > 0) {
    // Transfermarkt geöffnet: Wenn Spieler älter als 34 (29.08.16)
	...
	//if (($sql3['spiele_verein'] > 5 && $alter_in_jahren < 34) OR ($sql3['spiele'] <= 6 && $alter_in_jahren < 34)) {
		if ($_SESSION['transferGesperrt'] == FALSE) {
			...
			if ($sql3['transfermarkt'] == 0) {
				echo '<p>'._('Du kannst diesen Spieler auf dem Transfermarkt ins Ausland verkaufen oder ihn zur Leihgabe der Kauf anderen Vereinen anbieten. Wenn Du ihn ins Ausland verkaufst, wird er direkt gegen eine Ablöse verkauft. Wenn Du den Spieler zur Leihgabe oder zum Kauf anbietest, kannst Du später noch die Angebote unter Anfragen prüfen und entscheiden, ob Du eines davon annimmst.').'</p>';
					// Verkauf möglich nur an Spieltagen <=2 11,12 und >= 22 (29.08.16)
					if ((GameTime::getMatchDay() <= 2) || (GameTime::getMatchDay() == 11) || (GameTime::getMatchDay() == 12) || (GameTime::getMatchDay() >= 22)) {
						...
						echo '</select> <input type="hidden" name="spieler" value="'.$sql3['ids'].'" /><input type="submit" value="'._('Verkaufen ins Ausland').'" onclick="return'.noDemoClick($cookie_id, TRUE).' confirm(\''._('Bist Du sicher?').'\')" /></p>';
						echo '</form>';
					}
					else {
						echo '<p style="color:red;">'._('Transfermarkt geschlossen! Der Transfermarkt ist immer jeweils am ersten und letzten Spieltag der Hin- und Rückrunde geöffnet.').'</p>';
					...
					echo '</select> <input type="hidden" name="spieler" value="'.$sql3['ids'].'" /><input type="submit" value="'._('Anbieten zur Leihe').'" onclick="return'.noDemoClick($cookie_id, TRUE).' confirm(\''._('Bist Du sicher?').'\')" /></p>';
					echo '</form>';
					// Spieler verkaufen an andere Manager
					echo '<form action="/transfermarkt_aktion.php" method="post" accept-charset="utf-8">';
					echo '<p><select id="aukTyp" name="typ" size="1">';
					echo '<option value="999998">'.__('zum Kauf für %s €', number_format($sql3['marktwert'], 0, ',', '.')).'</option>';
					echo '</select> <input type="hidden" name="spieler" value="'.$sql3['ids'].'" /><input type="submit" value="'._('Anbieten zum Kauf').'" onclick="return'.noDemoClick($cookie_id, TRUE).' confirm(\''._('Bist Du sicher?').'\')" /></p>';
					echo '</form>';
			}
			else {
