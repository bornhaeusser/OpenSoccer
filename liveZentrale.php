...
if ($live_scoring_spieltyp_laeuft == '') {
	...
	// Nur für Liga-Spiele man_ligen joinen
	if ($live_scoring_spieltyp_laeuft == 'Liga') {
		$sql1 .= "JOIN ".$prefix."ligen AS c ON c.ids = b.liga ";
	}	
	...
	// Wenn Liga dann nach Namen von man_ligen sortieren sonst nach Liga von man_spiele sortieren
	if ($live_scoring_spieltyp_laeuft == 'Liga') {
		$sql1 .= "b.datum > ".$datum_min." AND b.simuliert = 1 AND b.typ = '".$live_scoring_spieltyp_laeuft."' AND a.minute < ".$live_scoring_min_gespielt." AND a.minute > ".$minMinute." ORDER BY c.name ASC";
	}
	else {
		$sql1 .= "b.datum > ".$datum_min." AND b.simuliert = 1 AND b.typ = '".$live_scoring_spieltyp_laeuft."' AND a.minute < ".$live_scoring_min_gespielt." AND a.minute > ".$minMinute." ORDER BY b.liga ASC";	
	}
	$sql2 = mysql_query($sql1);
	$resultTable = array();
	$letzterLigaWert = '';
